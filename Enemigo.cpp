...
Enemigo::Enemigo(int tipoE, float posx, float posy) {
    ...
    if (tipoE != 4) {
        ...
        if (tipoE == 3) {//enemigo mortero
            ...
            sonidoMuerte->setSonido("res/audio/dead2.wav");
            sonidoMuerte->getSonido().setLoop(false);
            sonidoMuerte->setReproduccion(0); //solo se reproducira 1 vez
        }
    } else {//vaca
...
void Enemigo::calcularColision(FloatRect** arrayColisiones, int nobjetos, FloatRect muerte) {
    bool colSuelo = false;
    bool colMuro = false;

    if (muerte.intersects(hitBox->getGlobalBounds())) {
        cout << "enemigocae" << endl;
        vidas = 0;
    }
...
void Enemigo::ColisionJugador(Jugador &jugador) {
    ...
    if (jugador.gethitBox().getGlobalBounds().intersects(hitBox->getGlobalBounds())) {
        ...
        if (Keyboard::isKeyPressed(Keyboard::A)) {
            if (RelojCuchillo.getElapsedTime().asSeconds() > 0.2) {
                restarVidas();
